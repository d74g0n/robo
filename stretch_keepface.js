...
board.on('ready', () => {
    ...
    let faceloop = setInterval(facelogicLoop, 100);
    ...
    look.sequence = [look.rndxy, look.up, look.left, look.down,look.rndxy, look.rndxy, look.rndxy, look.rndxy, look.rndxy, look.rndxy, look.rndxy, look.rndxy];
    _F.sequence = ['normie',
                   'shouting',
                   'shoutingcrazy',
                   'shoutinggonnacry',
    ...
    // PLAYING WITH COMMANDLINE ARGUMENTS:: WIP
    
    let cycles = 3;
    console.log(process.argv);
    console.log(`Command line args: ${process.argv.length}`);
    if (process.argv.length > 2) {
        console.log('detected commandline options:')
        console.log(`cycles: ${process.argv[2]}`);
    ...
    function sequenceloop(looptime = 1000) {
        return setInterval(function () {
            ...
            if (look.seqcount >= (look.seqmod * cycles) + 1) {
                clearInterval(faceloop);
                clearInterval(startupstretch);
                console.log(`warm up stretch cycles (${cycles}) complete`);
                //            console.log(`now double time!`);
                //                look.seqcount = 1;
                //                let loophalf = looptime;

                //WIP lets make the cycles longer but just a bit...
                //                loophalf+= 100;
                //                startupstretch = sequenceloop(loophalf);
                //                j5Seg7.clear();
                //                motors.stop()
    ...
    let startupstretch = sequenceloop(500);


//        masterTimer.init(motors, leftMotor, rightMotor);
//        masterTimer.start(240);
